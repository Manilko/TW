...
final class GuideController: UIViewController {
    ...
    private var filteredCollection: [Guide] = []

    // MARK: - Properties
    weak var sideMenuDelegate: SideMenuDelegate?
    weak var coordinatorDelegate: AppCoordinatorDelegate?
    weak var itemDelegate: GuidePresrntDelegate?

    private lazy var modView = GuideView(frame: UIScreen.main.bounds)

    init() {
        self.filterFlag = .all
        self.arrayMod = Array(RealmManager.shared.getObjects(Guide.self))
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        arrayMod.removeAll()
        arrayMod = Array(RealmManager.shared.getObjects(Guide.self))
        modView.modcCollectionView.reloadData()
    }

    private func filterCollection(_ collection: [Guide], by filterType: FilterType) -> [Guide] {
        switch filterType {
        case .all:
            return collection
        case .new:
    ...
    func setupSubViews() {
        ...
        modView.modcCollectionView.register(GuideCell.self, forCellWithReuseIdentifier: GuideCell.identifier)
    ...
    func creationRecommendedArray(from array: [Guide], number elements: Int) -> [Guide] {
        var modifiedArray: [Guide] = []
...
extension GuideController: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if collectionView == modView.filterView.collectionView {
            ...
            guard
                let cell = collectionView.dequeueReusableCell(withReuseIdentifier: GuideCell.identifier, for: indexPath) as? GuideCell
...
extension GuideController: UITextFieldDelegate {
