...
final class HouseIdeaController: UIViewController {

    var arrayMod: [HouseIdea]

    var filterFlag: FilterType
    private var searchResults: [HouseIdea] = []
    private var filteredCollection: [HouseIdea] = []

    // MARK: - Properties
    weak var sideMenuDelegate: SideMenuDelegate?
    weak var coordinatorDelegate: AppCoordinatorDelegate?
    weak var itemDelegate: HouseIdeaPresrntDelegate?

    private lazy var modView = HouseIdeaView(frame: UIScreen.main.bounds)

    init() {
        self.filterFlag = .all
        self.arrayMod = Array(RealmManager.shared.getObjects(HouseIdea.self))
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        arrayMod.removeAll()
        arrayMod = Array(RealmManager.shared.getObjects(HouseIdea.self))
        modView.modcCollectionView.reloadData()
    }

    private func filterCollection(_ collection: [HouseIdea], by filterType: FilterType) -> [HouseIdea] {
        switch filterType {
        case .all:
            return collection
        case .new:
    ...
    func setupSubViews() {
        ...
        modView.modcCollectionView.register(HouseIdeaCell.self, forCellWithReuseIdentifier: HouseIdeaCell.identifier)
    ...
    func creationRecommendedArray(from array: [HouseIdea], number elements: Int) -> [HouseIdea] {
        var modifiedArray: [HouseIdea] = []
...
extension HouseIdeaController: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if collectionView == modView.filterView.collectionView {
            ...
            guard
                let cell = collectionView.dequeueReusableCell(withReuseIdentifier: HouseIdeaCell.identifier, for: indexPath) as? HouseIdeaCell
...
extension HouseIdeaController: UITextFieldDelegate {
...
extension HouseIdeaController: UITableViewDataSource, UITableViewDelegate{
