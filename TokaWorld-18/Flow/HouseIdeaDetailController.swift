//
//  HouseIdeaDetailController.swift
//  TokaWorld-18
//
...
final class HouseIdeaDetailController: UIViewController {
    
    // MARK: - Properties
    weak var coordinatorDelegate: HouseIdeaDetailDelegate?
    let model: HouseIdea?
    let recommended: [HouseIdea]
    
    init(item: HouseIdea, recommended: [HouseIdea]) {
        self.model = item
        ...
        view().recommendedCollectionView.register(HouseIdeaRecommendedCell.self, forCellWithReuseIdentifier: HouseIdeaRecommendedCell.identifier)
    ...
    override func loadView() {
        super.loadView()
        self.view = HouseIdeaDetailView(isFavorite: model?.favorites ?? false)
    }

    override func viewDidLoad() {
        super.viewDidLoad()
        view().detailModsView.configure(with: model ?? HouseIdea())
    }
...
extension HouseIdeaDetailController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: HouseIdeaRecommendedCell.identifier, for: indexPath) as? HouseIdeaRecommendedCell else { return UICollectionViewCell() }
...
extension HouseIdeaDetailController: ViewSeparatable {
    typealias RootView = HouseIdeaDetailView
}
