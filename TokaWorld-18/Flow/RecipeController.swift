...
final class RecipeController: UIViewController {
    ...
    private var filteredCollection: [Recipe] = []

    // MARK: - Properties
    weak var sideMenuDelegate: SideMenuDelegate?
    weak var coordinatorDelegate: AppCoordinatorDelegate?
    weak var itemDelegate: RecipePresrntDelegate?

    private lazy var modView = RecipeView(frame: UIScreen.main.bounds)

    init() {
        self.filterFlag = .all
        self.arrayMod = Array(RealmManager.shared.getObjects(Recipe.self))
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        arrayMod.removeAll()
        arrayMod = Array(RealmManager.shared.getObjects(Recipe.self))
        modView.modcCollectionView.reloadData()
    }

    private func filterCollection(_ collection: [Recipe], by filterType: FilterType) -> [Recipe] {
        switch filterType {
        case .all:
            return collection
        case .new:
    ...
    func setupSubViews() {
        ...
        modView.modcCollectionView.register(RecipeCell.self, forCellWithReuseIdentifier: RecipeCell.identifier)
    ...
    func creationRecommendedArray(from array: [Recipe], number elements: Int) -> [Recipe] {
        var modifiedArray: [Recipe] = []
...
extension RecipeController: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
...
extension RecipeController: UITextFieldDelegate {
